...
class CalendarTideFactory {
    ...
    static func createTides(forYear year: Int, month: Int) -> [SDTide] {
        ...
        let tides: [SDTide] = SDTideFactory.tides(forStationName: station, withInterval: 900, forDays: Calendar.current.dateComponents([.day], from: interval.start, to: interval.end).day!, from: interval.start.startOfDay())
        return tides
    }
