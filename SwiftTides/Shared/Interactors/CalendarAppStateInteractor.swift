...
extension AppStateInteractor {
  func calculateCalendarTides(appState: AppState, settings: UserSettings, month: Int, year: Int) -> [SingleDayTideModel] {
    ...
    return SDTideFactory.tides(forStationName: station, withInterval: 900, forDays: Calendar.current.dateComponents([.day], from: interval.start, to: interval.end).day!, withUnits: units, from: interval.start.startOfDay()).map { SingleDayTideModel(tideDataToChart: $0, day: $0.startTime)}
  }
